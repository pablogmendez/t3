...
public class SummaryPrinter implements Runnable {
	...
	public void run() {
		...
		while (!Thread.interrupted()) {
			...
			// Imprimo el resumen
			System.out.printf(
					"Load Test Console: Resumen de ejecucion%n---------------------------------------%nTiempo de descarga promedio...: %d ms%nRequests exitosos.............: %d / %d %nRequests fallidos.............: %d / %d %nCantidad de usuarios..........: %d%nPresione ^C para terminar...%n",
					summary.getAvgDownloadTime(),
					summary.getSuccessfullrequest(), summary.getTotalRequests(),
					summary.getFailedrequest(), summary.getTotalRequests(),
					summary.getUsers());
			try {
				Thread.sleep((Long
						.parseLong(propertiesMap.get(Constants.SUMMARY_TIMEOUT))
