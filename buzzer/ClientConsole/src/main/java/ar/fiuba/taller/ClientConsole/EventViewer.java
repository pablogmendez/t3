...
public class EventViewer implements Runnable {
	...
	public EventViewer(BlockingQueue<Response> responseQueue, String username,
			String eventFile) {
	...
	public void run() {
		...
		try {
			while (true) {
				...
				pw = new PrintWriter(
						new BufferedWriter(new FileWriter(eventFile, true)));
			...
			logger.error(
					"Error al tomar la respuesta en la cola responseQueue");
